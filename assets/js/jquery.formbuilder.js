	...
	$.fn.formbuilder = function (options) {
		...
		var defaults = {
			...
			messages: {
				...
				label				: "Label",
				paragraph_field		: "Textarea Field",
		...
		return this.each(function () {
			...
			if (opts.load_url) {
				...
				*/
				$.ajax({
					type: 'post',
					url: ajaxurl,
					data: {action: opts.load_url},
					dataType: 'json',
					success: function(data) {
						form_db_id = data.form_id;
						fromJson(data.form_structure);
					},
					error: function(jqXHR, textStatus) {
						alert('Could not load form data: '+textStatus);
					}
				});
			}
			...
			var controlBox = function (target) {
					...
					box_content = '<select id="' + box_id + '" class="frmb-control" size="5">' + select + '</select>';
					save_button = '<input type="submit" id="' + save_id + '" class="button button-primary" value="' + opts.messages.save + '"/>';
					// Insert the control box into page
					if (!target) {
						$('#controls').append(box_content);
					} else {
			...
			var appendCheckboxGroup = function (values, options, required) {
					...
					field += '<div class="add-area"><a href="#" class="add add_ck" title="Add Option">' + opts.messages.add + '</a></div>';
					field += '</ol></div>';
					field += '</div>';
					field += '</div>';
					help = '';
			...
			var checkboxFieldHtml = function (values) {
					...
					field = '<li>';
					field += '<div>';
					field += '<input class="hide" type="checkbox"' + (checked ? ' checked="checked"' : '') + ' />';
			...
			var radioFieldHtml = function (values, name) {
					...
					field = '<li>'; 
					field += '<div>';
					field += '<input class="hide" type="radio"' + (checked ? ' checked="checked"' : '') + ' name="radio_' + name + '" />';
					field += '<input type="text" value="' + value + '" />';
					field += '<a href="#" class="remove" title="' + opts.messages.remove_message + '">' + opts.messages.remove + '</a>';
					field += '</div></li>';
			...
			var appendSelectList = function (values, options, required) {
					...
					field += '<div class="false-label">' + opts.messages.select_options + '</div>';
					field += '<div class="fields">';
					field += '<input type="checkbox" class="hide" name="multiple"' + (multiple ? 'checked="checked"' : '') + '>';
					field += '<label class="auto hide">' + opts.messages.selections_message + '</label>';
					...
					field += '<div class="add-area"><a href="#" class="add add_opt" title="Add Option">' + opts.messages.add + '</a></div>';
					field += '</ol></div>';
					field += '</div>';
					field += '</div>';
					help = '';
			...
			$('.frmb').delegate('.remove', 'click', function (e) {
				e.preventDefault();
			...
			var save = function () {
					if (opts.save_url) {
						$.ajax({
							...
							}) + "&form_id=" + form_db_id+"&action="+opts.save_url+"&thank-you-page="+$('#thank-you-page').val(),
							success: function(data){
								//console.log(JSON.stringify(data));
								//console.log('success: '+data.success);
								//var title = (data.success) ? 'Success' : 'Failed';
								var msg_str = '';
								if (data.messages) {
